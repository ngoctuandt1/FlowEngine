...
async def create_chain(req: ChainCreate):  # POST /api/chains
    ...
    for step in req.jobs:
        ...
        job = _build_job(step, profile=req.profile, chain_id=chain_id, job_level=level)
        await create_job(job)
        jobs.append(job)
        prev_id = job.id
        level += 1
    ...
    return {"chain_id": chain_id, "jobs": jobs}

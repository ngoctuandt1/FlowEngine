...
class RemoteAPI:
    ...
    async def list_pending_l3_siblings(
        ...
        *,
        ...
        """GET /api/jobs/l3-siblings — peek pending L3+ siblings for batch.
        ...
        if profile is not None:
            params["profile"] = profile
        resp = await self._request("GET", "/api/jobs/l3-siblings", params=params)
        if resp is None:
            return []
        try:
            resp.raise_for_status()
        except httpx.HTTPError:
            logger.warning("list_pending_l3_siblings non-2xx: %s", resp.status_code)
            return []
        data = resp.json()
        return data if isinstance(data, list) else []

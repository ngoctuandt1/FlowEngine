...
from flow.operations import remove_api
from flow.operations.remove_api import (
    clear_remove_capture,
...
REMOVE_URL = "https://aisandbox-pa.googleapis.com/v1/video:batchAsyncGenerateVideoObjectRemoval"
NON_REMOVE_URL = "https://aisandbox-pa.googleapis.com/v1/video:batchAsyncGenerateVideoObjectInsertion"
...
def _template_body(parent="old-parent", bbox=None):
...
def _client_with_template(body=None):
    client = _client()
    client._remove_request_template = {
        "url": REMOVE_URL,
        "headers": {
...
def _mock_recaptcha(monkeypatch):
    async def _mint(_page, *, caller):
        assert caller == "replay_remove_via_api"
        return "fresh-recaptcha-token"

    monkeypatch.setattr(remove_api, "_mint_recaptcha_token", _mint)


def test_install_remove_request_capture_records_latest_matching_post_template(caplog):
    page = FakePage()
    client = _client(page)
    ...
    assert client._remove_request_template == {
        "url": REMOVE_URL.upper(),
        "headers": {"authorization": "Bearer latest"},
        "post_data": '{"latest": true}',
        "anchored_parent": None,
    }
    assert "Captured Flow remove reverseAPI template" in caplog.text


def test_install_remove_request_capture_ignores_non_remove_urls():
    page = FakePage()
    client = _client(page)
    install_remove_request_capture(client)

    page.fire_request(SimpleNamespace(url=NON_REMOVE_URL, method="POST", headers={}, post_data="{}"))

    assert get_remove_request_template(client) is None


def test_install_remove_request_capture_replaces_previous_listener():
    page = FakePage()
    client = _client(page)
    install_remove_request_capture(client)
...
def test_get_remove_request_template_returns_none_then_dict_and_clear_resets():
    client = _client()

    assert get_remove_request_template(client) is None
    client._remove_request_template = {"url": REMOVE_URL, "headers": {}, "post_data": "{}"}
    assert get_remove_request_template(client) == client._remove_request_template

    clear_remove_capture(client)
    assert get_remove_request_template(client) is None


@pytest.mark.asyncio
async def test_replay_remove_via_api_raises_when_no_template_captured():
    with pytest.raises(RuntimeError, match="no captured template"):
        await replay_remove_via_api(_client(), "media/new-parent", BBOX)


@pytest.mark.asyncio
async def test_replay_remove_via_api_rewrites_body_and_sets_recaptcha(caplog):
    ...
    with caplog.at_level("INFO"):
        media_id = await replay_remove_via_api(client, "media/new-parent", BBOX)
    ...
    remove = request["videoObjectRemovalInput"]
    assert len(body["requests"]) == 1
    assert remove["sourceMedia"]["name"] == "media/new-parent"
    assert remove["bbox"] == BBOX
    ...
    assert "requests[0].videoObjectRemovalInput.sourceMedia.name" in caplog.text
    assert "requests[0].videoObjectRemovalInput.bbox" in caplog.text


@pytest.mark.asyncio
async def test_replay_remove_via_api_raises_on_4xx_with_body_snippet():
    client = _client_with_template()
    ...
    with pytest.raises(RuntimeError, match="HTTP 400: bad bbox"):
        await replay_remove_via_api(client, "media/new-parent", BBOX)


@pytest.mark.asyncio
async def test_replay_remove_via_api_raises_on_zero_media_entries():
    client = _client_with_template()
...
@pytest.mark.asyncio
async def test_replay_remove_via_api_headers_only_from_allowlist():
    client = _client_with_template()
    client.page.context.request.post.return_value = FakeAPIResponse(
        200,
...
@pytest.mark.asyncio
async def test_replay_remove_via_api_recaptcha_mint_failure_uses_captured_token(monkeypatch, caplog):
    async def _mint_empty(_page, *, caller):
        return ""

    monkeypatch.setattr(remove_api, "_mint_recaptcha_token", _mint_empty)
    client = _client_with_template()
    client.page.context.request.post.return_value = FakeAPIResponse(
        200,
        {"operations": [{"operation": {"name": "projects/proj1/media/new-media"}}]},
...
def _remove_body_unusual_path(parent_uuid: str) -> dict:
    return {
...
@pytest.mark.asyncio
async def test_replay_remove_via_api_walks_by_value_when_parent_path_candidates_miss(caplog):
    client = _client_with_template(_remove_body_unusual_path(PARENT_UUID))
    client._remove_request_template["anchored_parent"] = PARENT_UUID
    client.page.context.request.post.return_value = FakeAPIResponse(
        200,
    ...
    with caplog.at_level("INFO"):
        media_id = await replay_remove_via_api(client, NEW_PARENT_UUID, BBOX)

    assert media_id == NEW_PARENT_UUID
    body = json.loads(client.page.context.request.post.await_args.kwargs["data"])
    operation = body["requests"][0]["removeOperation"]

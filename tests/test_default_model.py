...
EXPECTED_DEFAULT_MODEL = "veo-3.1-lite"
...
def test_default_model_is_lite_across_server_frontend_and_flow():
...
def test_lp_aliases_removed_from_primary_model_choices():
    assert "veo-3.1-fast-lp" not in MODEL_MAP
    assert "veo-3.1-lite-lp" not in MODEL_MAP
    assert "veo-3.1-fast" in MODEL_MAP
    assert "veo-3.1-lite" in MODEL_MAP

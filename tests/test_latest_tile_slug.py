...
async def test_resolve_final_media_id_non_strict_prefers_latest_tile_when_url_stays_on_parent(
    monkeypatch, caplog
):
...
async def test_resolve_final_media_id_non_strict_overrides_when_url_is_clip_route_slug(
    monkeypatch,
):
...
async def test_resolve_final_media_id_non_strict_does_not_override_when_tile_slug_missing(
    monkeypatch,
):
    page = SimpleNamespace(url=_edit(PARENT_SLUG))
    monkeypatch.setattr(

...
### ~~B2 — Bbox không verify (P0)~~ ✅ FIXED via B11 (initial fix `a165105` targeted wrong element → superseded by B11 commit `<B11-COMMIT>`)
...
### ~~B11 — Bbox drag targets wrong element + overlay is canvas-painted (P0)~~ ✅ FIXED (commit `<B11-COMMIT>`)

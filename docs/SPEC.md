...
### ~~B38 — 1080p download never succeeded live; `_upsampled` API endpoint is permanently 404; modern UI requires UI-driven `/edit/` → icon Download → `1080pUpscaled` menu flow (P0)~~ ✅ FIXED (2026-04-19) · Tier2 Run 17f verified live (first `t2v_1080p_*.mp4` in project history, 38.4 MB)
...
- **Fix / live evidence:**
  ...
  - Log trace: `[UPSCALE] Clicking tile for SPA nav` → `SPA landed on /edit/` → `i-tag candidate buttons: 1` → `Clicked /edit/ download button (i-tag match)` → `Clicked 1080pUpscaled menu item` → `Upscaling in progress...` → `Complete (63s)` → `Saved: t2v_1080p_*.mp4 (38434315 bytes)`.
...
- Out of scope: B38 validated for L1 t2v only. Chain L2+ ops (extend/insert/remove/camera) with 1080p download not validated live; expected to land on `/edit/` already (no tile-click needed) but requires Run 18 confirmation. Also, `docs/session-reports/2026-04-19_download-probe.md` §5.5 stale claim about `fe_id_` = routing slug flagged for P3 doc fix.

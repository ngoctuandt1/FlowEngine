...
## C.1b — Chain Schema (B4)

```python
# server/models/chain.py

class Chain(BaseModel):
    """Immutable metadata row in the `chains` table.

    INSERT'd on POST /api/chains. Never UPDATE'd — status/progress are
    derived from the `jobs` table on every read (avoids drift).
    """
    id: str                           # UUID
    profile: Optional[str]            # pinned profile (may be None)
    created_at: datetime
    updated_at: datetime
...
class ChainAggregate(BaseModel):
    ...
    created_at: datetime
    status: str                       # derived — see rules below
    progress: ChainProgress           # {completed: int, total: int}
    jobs: list[str]                   # job ids in chain, ordered by created_at ASC
```

### Aggregated Status Rules (compute_aggregated_status)

Priority order — first match wins:

| # | Condition on member jobs' statuses | Chain status |
|---|---|---|
| 1 | any `failed` | `failed` |
| 2 | any `running` or `claimed` | `running` |
| 3 | any `pending` **and** any `completed`/`cancelled` | `running` (in-progress) |
| 4 | all `pending` | `pending` |
| 5 | all `cancelled` | `cancelled` |
| 6 | otherwise (≥1 completed, no failures) | `completed` |

Empty list (chain row with zero jobs) → `pending` (defensive default).

The `chains.status` DB column is **vestigial** — stays at INSERT default
`'active'`. Not surfaced through the API; do not read it in application code.
...
POST /api/chains
  ...
  Response: 201 {chain_id: str, jobs: Job[]}
  Side-effect (B4): INSERTs a row into `chains` table with id + profile.

GET /api/chains/{id}
  Response: 200 ChainAggregate | 404
  ChainAggregate = {id, profile, created_at, status, progress, jobs}
  status + progress computed on-demand from jobs GROUP BY chain_id (B4).
...
### D.3.6 — ~~Chains table unused (B4)~~ ✅ FIXED
`chains` table giờ INSERT mỗi khi POST /api/chains. Metadata (id + profile + created_at) immutable — không UPDATE path. Aggregated status compute on-demand từ `SELECT chain_id, status FROM jobs GROUP BY chain_id` (Choice C hybrid, xem §D.4 B4).
...
### ~~B4 — Chains table không dùng (P2)~~ ✅ FIXED (commit `4dcf50f`)
- File: `server/db/database.py:12-20` (schema unchanged), `server/db/chain_store.py` (NEW), `server/models/chain.py` (NEW), `server/routes/jobs.py` (POST /api/chains + GET /api/chains/{id}).
- Triệu chứng: `chains` table được CREATE trong schema nhưng `POST /api/chains` chỉ tạo jobs với shared `chain_id`, không INSERT vào chains table — FK `jobs.chain_id → chains.id` orphan. Frontend muốn chain-level progress phải GROUP BY `jobs.chain_id` mỗi render.
- **Resolution (Choice C — Hybrid, xem session report §7):** `POST /api/chains` gọi `chain_store.create_chain(Chain(id, profile))` — INSERT immutable metadata (id + profile + created_at + updated_at). KHÔNG có UPDATE path từ `update_job` đến `chains.status` — tránh drift giữa chains row và jobs rows. Column `chains.status` stay ở DEFAULT `'active'` (vestigial, không surface qua API). `GET /api/chains/{id}` trả `ChainAggregate{id, profile, created_at, status, progress, jobs}` với `status` + `progress` **derived** mỗi request từ `SELECT id, status FROM jobs WHERE chain_id = ? ORDER BY created_at ASC`. Aggregation rules (priority): (1) any `failed` → failed; (2) any `running`/`claimed` → running; (3) any `pending` — nếu có completed/cancelled khác thì running (in-progress), còn không thì pending; (4) all `cancelled` → cancelled; (5) otherwise → completed. Single source of truth = `jobs` table.
- Rejected alternatives: **Choice A (full sync)** — UPDATE `chains.status` mỗi `update_job` terminal: 2 nơi store cùng truth → drift risk nếu transaction fail giữa 2 UPDATE. **Choice B (view-only, không INSERT)** — chains table vẫn unused → idiom `CREATE TABLE never INSERT'd` không giải quyết.
- Guard: `tests/test_chains.py` (17 cases):
  - 6 unit cho `compute_aggregated_status` (pending, failed-wins, running/claimed, completed, mixed pending+completed→running, empty→pending)
  - 2 DB cho `create_chain` + `get_chain_row` (None cho unknown)
  - 7 API cho POST + GET (row persisted, aggregate pending/completed/failed/running/mixed, 404 unknown, job-ids-ordered)
  - 1 trip-wire `test_chain_status_not_synced_from_job_update` — drive job `pending → running → completed` và assert `chains.status` không đổi (Choice C invariant)
  - 1 ordering `test_get_chain_aggregate_includes_job_ids_in_order`
- Schema: unchanged (minimal diff, Choice C không cần column mới). Session report: `docs/session-reports/2026-04-18_B4_chains-table.md`.
...
### ~~B18 — Homepage `+ New project` selector locale-hardcoded → every T2V on VI/non-EN account fails (P0)~~ ✅ FIXED (commit `8dc357c`)
...
### ~~B19 — Aspect-ratio chip selector breaks when (a) default model is not `"Video"` or (b) Radix trigger is pre-open (P0)~~ ✅ FIXED (commit `e1597b2`)
- Discovered during Tier 2 2026-04-18 live E2E on `ngoctuandt20` after B18 unblocked code flow past homepage — `docs/E2E_RESULTS_PHASE_A.md` Runs 3-6 + `docs/session-reports/2026-04-18_B19_aspect-chip-multiline.md`.
- File: `flow/operations/generate.py` → `_set_aspect_ratio()` (same function fixed by B1; B19 is a follow-up bug class that B1's Tier 1 tests could not catch because the mocked chip text was always `"Video ... x1"`).
- Triệu chứng: `text_to_video(aspect_ratio="9:16")` raised `Locator.wait_for: Timeout 3000ms exceeded` on `[role="menu"][data-state="open"]`. Engine halted at `_set_aspect_ratio` before reaching submit, blocking Tier 2 validation of B1 end-to-end.
- Two-part root cause (live DOM probe via Chrome MCP on the failing project):
  1. **Stale model-name text probe.** Pre-B19 locator used `re.compile(r"video.*x\d", re.IGNORECASE)` on the chip's `innerText`. Live chip under a non-default model renders `<model-name>\ncrop_X_Y\nxN`, e.g. `"🍌 Nano Banana Pro\ncrop_9_16\nx1"` — regex matches nothing. Phase A Tier 1 never hit this because `ngoctuandt20`'s default model was migrated to Nano Banana Pro between tier runs; previously it was `"Video"`.
  2. **Pre-open Radix state.** The chip's DropdownMenu trigger is a Radix `data-state` element — `"closed"` vs `"open"`. On current Flow DOM, an earlier interaction (likely `flow/model_selector.py` which opens via `button:has-text('Video')` — a selector that happens to also match the aspect chip's old-DOM label) leaves the aspect chip's trigger with `data-state="open"` by the time `_set_aspect_ratio` runs. Pre-B19 code unconditionally clicked the chip, which TOGGLED the already-open menu CLOSED, so the immediately-following `wait_for("[role=\"menu\"][data-state=\"open\"]")` timed out. Live diag `DIAG aspect chip: ... dataState: 'open'` prefixing each retry in Run 6 confirmed this.
- Runtime effect (pre-fix): **every `text_to_video` job with `aspect_ratio="9:16"` on any account whose default model is not exactly `"Video"` failed** at the chip open step, independent of locale. Tier 2 Runs 3/4/5 all hit the same timeout; fix v1 (icon-ligature selector only) still failed Run 4/5 because the click-toggle effect was independent of which selector resolved the chip; Run 6 with live diag surfaced the pre-open state; Run 7 with both fixes passed.
- **Resolution (commit `e1597b2`):**
  - **Icon-ligature CSS `:has-text` selector (R-CODE-3 Locale-Independent).** Replace the model-name regex with `button[aria-haspopup="menu"]:has-text("crop_9_16"), button[aria-haspopup="menu"]:has-text("crop_16_9")`. The Material Icon ligature text (`crop_9_16` / `crop_16_9`) is baked into the chip's inner `<i class="google-symbols">` and is stable across models, locales (VI / EN), and font-loading states. Avoids multi-line / anchor regex edge cases entirely.
  - **Pre-open guard.** Before clicking the chip, read `chip_btn.get_attribute("data-state", timeout=2000)`. If already `"open"`, skip `chip.click()` and fall through directly to the existing `[role="menu"][data-state="open"]` wait. Radix triggers with `aria-haspopup="menu"` expose `data-state` as a first-class attribute — no feature detection needed.
  - Both parts are necessary. The selector fix alone reliably finds the right element; the state guard alone doesn't help if the selector matches the wrong element. Together they survive any combination of default-model choice and prior DOM state.
- Guard: `tests/test_aspect_ratio.py` — 5 cases (3 carried over from B1, 2 new):
  - `test_default_ratio_no_interaction` — untouched (B1).
  - `test_unsupported_ratio_logs_warning` — untouched (B1).
  - `test_portrait_opens_panel_and_clicks_trigger` — happy path (chip `data-state` unset → click fires, portrait trigger clicked, chip verified). Original B1 test, still passes under the new selector.
  - `test_portrait_skips_chip_click_when_already_open` — **NEW B19 core invariant.** `_make_locator(get_attribute="open")` → `chip.click.assert_not_called()` while `portrait_trigger.click.assert_called()` + `page.mouse.click(10, 10)` + `chip.inner_text` all fire. Prevents future regression to the unconditional-click form.
  - `test_chip_selector_uses_icon_not_model_text` — **NEW source trip-wire.** Three guarantees: (1) source contains `"crop_9_16"` and `"crop_16_9"` plus `aria-haspopup="menu"`, and does NOT contain `video.*x` text (trip-wire); (2) pre-B19 regex `r"video.*x\d"` with `DOTALL` does NOT match the live Run 3 chip text `"🍌 Nano Banana Pro\ncrop_9_16\nx1"` (anchors the bug hypothesis); (3) ligature regex `^crop_(9_16|16_9)$` matches the two supported aspect-ratio ligatures and rejects unrelated tokens (`add_2`, `crop_free`, `crop_rotate`, `video_library`).
- Live verdict: **Tier 2 Run 7 ✅** — J1 `text-to-video` (aspect 9:16) on `ngoctuandt20` reached `completed`, persisted `media_id=f2f736d2-5094-4bdb-abc6-d4f8ed254ccb` + `project_url=https://labs.google/fx/tools/flow/project/f656f223-7e65-4309-bc34-cd39e9b3da24`. Run 8 (3-job chain) J1 also succeeded; J2/J3 halted on an independent L2 inheritance gap (parent `project_url`/`media_id` not propagated in `server/db/job_store.py::claim_next_job`) — out of B19 scope, flagged for future B-candidate.
- Session report: `docs/session-reports/2026-04-18_B19_aspect-chip-multiline.md`.
- Follow-up candidates discovered but NOT fixed:
  - ~~**B20 (P2 proposed):** `flow/model_selector.py` uses `button:has-text('Video')` to open the model dropdown — on current DOM the same selector also matches the aspect chip. Model selector succeeds via Playwright `.first` ordering but is the origin of the pre-open state B19 now tolerates. Propose making model selector use a non-text anchor (icon or `role=combobox`).~~ ✅ **FIXED** — primary `'Video'` text collision absorbed by B26 (commit `d4fca1a`, `chip_selectors` rewritten to `aria-haspopup='menu'` + exact icon ligatures); three residual fuzzy `'Veo'` sites (`_open_model_dropdown` line 281 + `get_current_model` lines 556-557) converted to the same canonical pattern per `docs/FLOW_BUTTON_EXACT.md §1.6` in commit `0aa01b8`: `button[aria-haspopup='menu']:has(i:text-is('arrow_drop_down'))` + `.filter(has_text=re.compile(r"^Veo", re.IGNORECASE))`. Trip-wire `tests/test_model_selector.py::test_no_fuzzy_veo_selector` blocks regression (forbids both `has-text('Veo')` CSS substring and raw `filter(has_text="Veo")` string filter). Full suite 93 pass (was 92 + 1 new), zero DeprecationWarning under `-W error::DeprecationWarning`.
  - ~~**B21 (P3 proposed):** Worker emits a stray bare `arrow_drop_down` line to stdout during model selection (stray `print()` in `flow/model_selector.py`).~~ ✅ **Self-resolved** — `grep "print(" flow/model_selector.py` at master `83f621f` returns zero hits; the stray `print()` reported in B19 follow-ups is not present in the current code (most likely removed incidentally during an earlier B17/B26 refactor). Marked closed 2026-04-19 with the B20-final commit `0aa01b8`. No test added — there is no source artefact to trip-wire against; `flow/model_selector.py` passes `flake8 --select=T201` (print statement lint) by construction.

### ~~B22 — L2+ claim does NOT inherit `project_url` / `media_id` / `edit_url` from parent (P0)~~ ✅ FIXED (commit `0637c92`)
...
### ~~B26 — Fuzzy `:has-text` selectors on Material Icon tokens silently redirect /edit/ → /project/ between model selection and submit (P0)~~ ✅ FIXED (commit `d4fca1a`)
- Discovered during Tier 2 2026-04-19 live-extension E2E verification of the B22 fix on `ngoctuandt20`. Symptom: `submit_with_confirmation` timed out with `new_api_calls=0, gen_id=None, url=https://…/project/<id>` on every L2 extend job. URL in the timeout log is the tell — the submit was running on /project/, not /edit/.
- Files (three):
  - `flow/model_selector.py` — `_switch_to_video_tab` JS fallback, `chip_selectors` list, `_has_dropdown_arrow`
  - `flow/submit.py` — `SUBMIT_SELECTORS` list, `_SKIP_PATTERN` (removed), `click_submit` signature gains `scope=`
  - `flow/operations/_base.py` — `click_action_button` reworked with `_MODE_ICON_BY_TITLE` map
- Root cause (three sites, same class — fuzzy `:has-text`/substring leaks across concatenated icon+label buttons):
  1. **Primary (`_switch_to_video_tab` JS fallback).** On /edit/, the Playwright selector `[role='tab']:has-text('Video')` misses the Radix tab (its `textContent` is `'Video'` exactly, the selector was being blocked by visibility timing in the test DOM). The JS fallback used `const lower = (el.innerText || '').toLowerCase(); if (lower === 'video' || lower === 'videocam' || lower.includes('videocam')) el.click();`. On the /edit/ bottom-strip the **Camera mode-switch button** has `innerText === 'videocam\nCamera'` (Material Icon ligature on one line, label on the next). `lower.includes('videocam')` matched it; the `!lower.match(/x[1-4]/i)` guard passed (no `x1`/`x4`); the click toggled Camera mode on. Flow's SPA then redirected /edit/ → /project/ and changed the composer's `data-scroll-state`. By the time `submit_with_confirmation` ran, the L2 extend composer was gone; the submit it found was the L1 /project/ generate button, which did not accept the L2 extend payload, so the POST to `/operations/` never fired.
  2. **Secondary (`flow/submit.py`).** `button:has(i:has-text('arrow_forward'))` also used fuzzy `:has-text`. Largely masked by the `is_enabled` check + the `_SKIP_PATTERN` text filter, but structurally the same leak class — `has-text` matches `arrow_forward_ios` / `arrow_forward_circle` variants too.
  3. **Secondary (`flow/operations/_base.py::click_action_button`).** `button:has-text('Chèn')` matched decorative icon buttons that contained `'Chèn'` as a substring of a tooltip `aria-describedby` reflected text. Rare but possible.
- Runtime effect (pre-fix): **every L2 extend job** (and likely some L2 insert/remove jobs that happened to tab-open the model panel) failed with silent submit-confirmation timeout. The L2 operation appeared to run normally until the submit step, where it timed out without any API activity. Tier 2 Run 9 (the intended B22 verification run) was the first time B22+ progressed past the claim branch into the UI; B26 surfaced immediately on that first real attempt.
- **Resolution (commit `d4fca1a`):** exact-text everywhere — use Playwright's `:text-is(...)` engine (exact, trimmed match) on the Material Icon `<i>` child's textContent. Icon ligatures are designed by Google to be stable across locales, font-loading states, and Flow UI redesigns.
  - **`flow/submit.py`.** Collapse `SUBMIT_SELECTORS` to a single entry: `"button:has(i:text-is('arrow_forward'))"`. Remove `_SKIP_PATTERN` (redundant under exact-text — the selector no longer matches non-submit buttons). Add `scope: str | None = None` parameter to `click_submit` (and pass-through to `submit_with_confirmation`); when given, the CSS scope is prepended to the base selector (`"{scope} {base}"`) so L2 callers can restrict search to the panel's `data-scroll-state='START'` scope if needed. On /edit/ there are 2 buttons matching the exact-text selector (one disabled decorative + one enabled real submit); the B16 iteration + `is_enabled` check already picks the right one, so no new scoping logic needed in the extend/insert/remove/camera callers for now.
  - **`flow/model_selector.py`.** `chip_selectors` rewritten to `button[aria-haspopup='menu']` scoped to the crop-icon ligatures (`crop_16_9`, `crop_9_16`, `crop_1_1`) for /project/ composer + the `arrow_drop_down` ligature for /edit/ composer (live probe 2026-04-19 confirmed no crop_* icons on /edit/, only `arrow_drop_down`). `_has_dropdown_arrow` now uses `i:text-is('arrow_drop_down')` (fuzzy would match `arrow_drop_down_circle` distractors). `_switch_to_video_tab` Playwright path uses `[role='tab']:text-is('Video')` + `[role='tab']:has(i:text-is('videocam'))` fallback; JS fallback now **blacklists the mode-switcher button titles** (`'Camera'`, `'Mở rộng'`/`'Extend'`, `'Chèn'`/`'Insert'`, `'Xoá'`/`'Xóa'`/`'Remove'`/`'Delete'`) before any match, then requires exact `textContent === 'Video'` on a `role='tab'` element OR an `<i>` child with exact `textContent === 'videocam'`. The B26 bug cannot recur because the mode-switcher titles are explicitly excluded, and the match conditions are strict-equals, not substring.
  - **`flow/operations/_base.py`.** Add `_MODE_ICON_BY_TITLE` dict mapping each mode title (both VI and EN) to its unique Material Icon ligature:
    - `Mở rộng`/`Extend` → `keyboard_double_arrow_right`
    - `Chèn`/`Insert` → `add_box`
    - `Xoá`/`Xóa`/`Remove`/`Delete` → `ink_eraser`
    - `Camera` → `videocam`
    `click_action_button` now does two passes: pass 1 tries `button[title='{text}']` (VI labels are unique, stable); pass 2 falls back to `button:has(i:text-is('{icon}'))` with the mapped icon. Both passes use exact matches — no more `:has-text('Chèn')`.
- Guard: `tests/test_submit.py` reworked for B26:
  - **Removed** `test_click_submit_skip_pattern_preserved` (the `_SKIP_PATTERN` contract no longer exists)
  - **Removed** `test_click_submit_no_enabled_button` + `test_click_submit_zero_count_falls_through` (both exercised fall-through from `SUBMIT_SELECTORS[0]` to `SUBMIT_SELECTORS[1]`, which the B26 collapse removes)
  - **Preserved** 5 B16 KEEP-7 tests (iteration, skip-disabled-first, debug log, keyboard fallback, per-button exception resilience)
  - **NEW** `test_click_submit_scope_prepends_to_selector` — verifies `scope=` kwarg prepends the CSS scope to the base selector before calling `page.locator`
  - **NEW** `test_submit_selectors_use_exact_text_and_no_fuzzy_antipattern` — source trip-wire: verifies canonical selector `button:has(i:text-is('arrow_forward'))` is present; forbids fuzzy anti-pattern `button:has(i:has-text('arrow_forward'))`; forbids 3 text-based aria-label probes (`Create`, `Generate`, `Send`); pins `len(SUBMIT_SELECTORS) == 1` with a comment explaining that additions must extend scope handling + this test
  - Full suite: 92 pass (was 93 pre-B26; net −1 from −3 obsolete + 2 new). Zero DeprecationWarning under `-W error::DeprecationWarning`.
- Live verdict: **PASS** on `ngoctuandt20` VI profile via Chrome extension, 2026-04-19:
  - **L2 extend** — navigated to /edit/ff3c33e0, model chip matched via new `arrow_drop_down` selector (unique on /edit/), selected `Veo 3.1 - Lite [Lower Priority]`, URL stayed on /edit/ (B26 bug did NOT recur), submit matched via new exact-text selector, 18% progress, composer cleared, new clip appeared in history panel.
...
### B32 — `navigate_to_edit` doesn't activate target clip; B30 walk-up alone insufficient (P0, revises B30) ✅ FIXED (2026-04-19) · Tier2 Run 12 verified live 2026-04-19
- Root cause: B30 (combined commit `fc31a54`) walks up past `extend-video` ancestors for BOTH `media_id` AND `edit_url`. But navigating `page.goto(/edit/{L1_grandparent_media})` after a sibling extend-video completes still triggers the B29 SPA-strip (verified live 2026-04-19 on project `513d580b-7def-46d6-adba-e85663b84539`: `goto /edit/6842325d-…` → URL rewritten to `/project/{id}` library grid with zero mode buttons). B30 walk-up for `edit_url` targeted the stale URL; B29 guard raises but provides no recovery path.
- **Live evidence (2026-04-19 B32 probe):** on `/edit/1a6e3b77-…` (extend-output, B28 locked — Insert/Remove/Camera `disabled=true`), the right history panel renders each project clip as `<div data-tile-id="fe_id_{media_id}">`. Dispatching a full `MouseEvent` sequence (`pointerdown → mousedown → pointerup → mouseup → click`) on the tile for the L1 parent media **flips all 4 mode buttons from disabled → enabled** without changing `page.url`. Plain `tile.click()` doesn't fire Flow's styled-components handler — must dispatch explicit events.
- **Fix (`flow/operations/_base.py`):**
  - **Split B22 / B30 semantics in `claim_next_job`:**
    - `edit_url` comes from the **DIRECT parent** (pure B22 propagation, never stale — the parent just completed on that URL).
    - `media_id` keeps the B30 walk-up to the nearest non-extend ancestor (the SEMANTIC target the L3+ op wants to edit).
  - New `_activate_clip_tile(page, media_id)` helper: `await page.locator('[data-tile-id=\\'fe_id_{media_id}\\']').first.wait_for(state='attached')` then `page.evaluate` dispatches the full `MouseEvent` sequence via JS. Returns True on success, False on missing tile / empty media_id.
  - `navigate_to_edit` now: after the post-goto `/edit/` check passes, if `extract_media_id(page.url) != job.media_id` (target ≠ URL's media), invoke `_activate_clip_tile(page, job.media_id)` to switch Flow's active clip to the target. Non-fatal: if the tile isn't found, logs a warning and the caller's mode-button click will surface the B28 extend-child-lockout error with the correct diagnostic.
- **Chain behavior post-B32:**
  - L1 t2v → L2 extend → L3 insert: worker claims with `edit_url=/edit/{L2_extend_output}` (fresh) + `media_id=L1.media` (target). Goto L2's URL, verify /edit/ intact, activate L1 tile → Insert sidebar enabled → proceed.
  - L1 t2v → L2 camera → L3 insert: `edit_url=/edit/{L2_camera}` + `media_id=L2.media` (walk-up is a no-op for camera). URL media == target media → tile activation is skipped — same as pre-B32 behavior.
...
### ~~B38 — 1080p download never succeeded live; `_upsampled` API endpoint is permanently 404; modern UI requires UI-driven `/edit/` → icon Download → `1080pUpscaled` menu flow (P0)~~ ✅ FIXED (2026-04-19) · Tier2 Runs 17f + 18 verified live — **4/4 live pass** (first `t2v_1080p_*.mp4` in project history; Run 18 3/3 stability across LANDSCAPE + PORTRAIT)
- Symptom: pre-B38 historical count of `t2v_1080p_*.mp4` across every Tier 2 run ever (Runs 10–16): **0 files**. B34 (poll window 30→180s) and B34b (retries 12→24 = 360s window) both failed to produce a 1080p file — every run still fell through to `t2v_720p_*.mp4` API fallback.
- Root cause (per 2026-04-19 download probe §5.4): the `_upsampled` endpoint `media.getMediaUrlRedirect?name={media_id}_upsampled` returns **HTTP 404 permanently** — not "still upscaling, retry later". Every poll-window bump was architecturally dead. Modern Flow UI triggers upscale via `POST aisandbox-pa.googleapis.com/v1/flow/uploadImage` — only reachable by clicking the icon-only `<button><i>download</i></button>` on `/edit/{routing_slug}` → Radix menu `[role='menuitem']` text `1080pUpscaled`.
- Additional blockers surfaced during Run 17 live validation:
  1. **Post-L1 page is `/project/{pid}` root**, not `/edit/` — must navigate first.
  2. **UUID dualism + SPA coupling** (probe §5.5 + Runs 17d/17e): neither the API `media_id` nor the tile's `data-tile-id="fe_id_{X}"` suffix is the `/edit/` routing slug. `page.goto(/edit/{either})` is bounced to project root by Flow's SPA. Only `tile.click()` — through the SPA's own `pushState` + router state setup — settles on the correct `/edit/{routing_slug}`. Probe §5.5's "`fe_id_` prefix = routing slug" claim is stale/wrong (Run 17e evidence: `fe_id_54ce98c9…` vs post-click URL `edit/4ed94c32…`).
  3. **Google login overlay** `<div class="dKGsO" jsname="OQ2Y6">` intercepts pointer events, blocking `input#identifierId` clicks indefinitely — surfaced Run 17a. User feedback: "load lại url là bypass".
- Files:
  - **NEW**: `flow/upscale.py` (273 lines) — cherry-picked + async-ported from `AI-Engine3-Project/modules/upscale_unified.py`. Public: `upscale_and_download_1080p(client, prefix, output_dir, upscale_timeout_sec, max_retries=2, media_id=None)`. Internal: `_ensure_edit_view` (tile.click SPA nav), `_click_edit_download_button` (i-tag Material Icons match + text-regex fallback), `_click_menu_1080p` (anchored `^1080pUpscaled$` — excludes `4KUpscaled · 50 credits`), `_wait_upscale` (VI+EN toast regex poll), `_close_toast` (Close button only, NEVER Escape per CLAUDE.md §7), `_save_download`, `_redownload_1080p`.
  - **MOD**: `flow/download.py::download_video` — if `quality=="1080p"`, call `upscale_and_download_1080p` as primary (extracts `first_mid` from `media_ids[0]` or `client._media_id_events`). On failure, short-circuit to 720p API (skip stale `_upsampled` poll → saves ~6min per fallback).
  - **MOD**: `flow/login.py::handle_login_redirect` — stuck-detect: if `(step, url)` same for ≥3 iterations AND `step in (email / password / totp / challenge_select)`, `await page.reload(wait_until="domcontentloaded")` before retrying. Also saved as persistent memory `feedback_login_stuck_reload.md`.
- **Fix / live evidence:**
  - Run 17f (2026-04-19 20:33:03–20:34:13), job `581287d2-8eeb-4ae0-9d65-fedfe6ad096c` → `downloads\t2v_1080p_1776605652.mp4` **38 434 315 B / 38.4 MB**. Upscale proper: 63s (busy toast → done toast). See `docs/session-reports/2026-04-19_Tier2_Run17_B38_UI_upscale.md` for full iteration log (17a login stuck → 17f tile.click success, 6 sub-runs).
  - **Run 18 (2026-04-19 22:07–22:16) — stability 3/3 PASS**: J1 `22c84f49` 16:9 lighthouse → `t2v_1080p_1776611477.mp4` 10.9 MB (66s), J2 `09fd6ef7` 9:16 violin → `t2v_1080p_1776611618.mp4` 11.9 MB (51s), J3 `87b26c12` 16:9 river → `t2v_1080p_1776611789.mp4` 32.6 MB (96s). ffprobe on all 4 files (17f + 18): **1920×1080 h264 24fps 8.0s**. Orientation-agnostic (LANDSCAPE + PORTRAIT both land /edit/ cleanly, no 720p fallback). See session report §11 for full addendum.
  - Log trace: `[UPSCALE] Clicking tile for SPA nav` → `SPA landed on /edit/` → `i-tag candidate buttons: 1` → `Clicked /edit/ download button (i-tag match)` → `Clicked 1080pUpscaled menu item` → `Upscaling in progress...` → `Complete (51–96s)` → `Saved: t2v_1080p_*.mp4`.
- Guard: no new unit tests added (DOM-dependent UI flow can't be mocked meaningfully; live E2E is the definitive verification). The `1080pUpscaled` anchor regex `^1080pUpscaled$` is a trip-wire against accidentally matching `4KUpscaled · 50 credits` (50 LP per click). `_close_toast` whitelist `^(close|dismiss|đóng)$` — anchored, locale-independent.
- Full-suite verification: **119 pass** (unchanged — all pre-B38 tests pass; no new `.py` under `tests/` needed since fix is UI-behavior-dependent).
- Out of scope: B38 validated for L1 t2v stable (Run 17f + Run 18 = 4/4). Chain L2+ ops (extend/insert/remove/camera) with 1080p download not yet live-verified (deferred to **Run 19**); expected to land on `/edit/` already (via B32 tile activation in `navigation.py`) so `_ensure_edit_view` should short-circuit at the `if "/edit/" in page.url: return` guard. Also, `docs/session-reports/2026-04-19_download-probe.md` §5.5 stale claim about `fe_id_` = routing slug flagged for P3 doc fix.
- Supersedes: B34 (30→180s poll window) and B34b (180→360s retry bump) — both were patches to a dead endpoint. B34b's 360s window + `test_upscale_poll_defaults_meet_minimum` trip-wire remain as defense-in-depth for the 720p-path transient retry, but the `_upsampled` poll is now skipped entirely in the 1080p→720p fallback (`api_quality = "720p"` short-circuit in `download.py`).

### ~~B37 — `download_video` reads wrong key from `_media_id_events`; primary harvest path silently empty → blob fallback (P1)~~ ✅ FIXED (2026-04-19) · Tier2 Run 14 surfaced + fix verified
- Symptom: Run 14 (2026-04-19 18:00, job `da2eb837`) — B35 `chip verified x1` log fired and `files=1` confirmed, but `text-to-video DONE | files=1 media_id=None` + `[WARNING] flow.download: No media IDs found for download` → fell through to `_download_blob` → saved `downloads/t2v_blob_1776596510.mp4` (1.9 MB, video-element capture, not API). Run 13 same-day on same account/model succeeded (17.9 MB `_720p_` + `media_id=7c4fa302`) — non-deterministic.
- Root cause (2-part):
  1. `flow/download.py::download_video` harvested network-captured media IDs via `evt["media_id"]`, but `flow/client.py::_record_media_id` stores each event as `{"mid": ..., "source": ..., "url": ..., "ts": ...}`. Key mismatch (`media_id` vs `mid`) → the primary harvest list comprehension was always empty. Every production download has been depending on fallback #2 (`_video_urls`) since the feature landed.
  2. `_video_urls` is a list of `{"url": str, "ts": float}` dicts (see `client.py::_on_response` line 466). Fallback #2 iterated `for url in ...` passing each **dict** (not the URL string) into `media_id_from_url(url)`. That function did `str(url or "")` internally — the regex `[?&]name=(...)` matched the dict's Python repr by coincidence when the captured URL carried `?name=…`, but missed on captures that were direct `.mp4` / blob URLs.
- Why Run 13 passed, Run 14 failed: Run 13's `_video_urls` happened to include a `getMediaUrlRedirect?name=…` call before completion fired → stringified-dict regex accidentally matched. Run 14's captures only carried direct video URLs (no `?name=`) → empty list → blob fallback.
- File: `flow/download.py:49-67` — the 2-part harvest block.
- **Fix:**
  - `download.py:50` → read `evt["mid"]` instead of `evt["media_id"]`.
  - `download.py:62-66` → unwrap `entry["url"]` before calling `media_id_from_url(url)` (with `isinstance(entry, dict)` tolerance for future schema).
- Guard: `tests/test_download.py` +2 source trip-wires:
  - `test_download_reads_mid_key_from_media_id_events` — asserts source contains `evt["mid"]` (or single-quoted). Prevents rename regression on either side.
  - `test_download_unwraps_video_urls_list_of_dicts` — asserts source unwraps `entry["url"]` (or does the `isinstance` check). Prevents the accidental-string-coercion path from returning.
- Full-suite verification: **119 pass** (was 117 + 2 new B37 tests). Zero DeprecationWarning under `-W error::DeprecationWarning`.
- Not in scope: the download pipeline's retry/upscale-latency envelope (covered by B34). B37 is strictly the harvest-key correctness fix.

### ~~B35 — Engine relies on Flow's per-account x1/x2/x3/x4 default; silent credit leak on accounts defaulting to x≥2 (P0)~~ ✅ FIXED (2026-04-19) · Tier2 Run 13 verified live 2026-04-19 (job `45f7ccf6`, `files=1 media_id=7c4fa302`, chip verified `x1`) · Tier2 Run 14 re-verified live 2026-04-19 (job `da2eb837`, `files=1`, chip verified `x1` — surfaced B37)
...
### ~~B34 — 1080p download never succeeds live; `_upsampled` poll window (30s) too short for Flow's upscale latency (P1)~~ ✅ FIXED (2026-04-19) · B34b retry-count bump (2026-04-19, Run 15 residual)
...
- **B34b (2026-04-19, Run 15 residual P3 close-out):** Run 15 showed all 3 t2v jobs still fell through to 720p at 180s window → bumped default `UPSCALE_MAX_RETRIES` 12 → **24** (total `15 × 24 = 360 s` = 6 min). Evidence: Run 15 commit `81e27bd` — 3/3 `t2v_720p_*.mp4`, 0 `t2v_1080p_*`. Flow's upscale on 16:9 + 9:16 fast-LP clips is longer than the initially estimated 1-3 min envelope. Env override `FLOW_UPSCALE_MAX_RETRIES` preserved — ops can tune further without code change. Trip-wire `test_upscale_poll_defaults_meet_minimum` updated: floor raised 120s → **300s** (60s margin above the last observed miss). 720p fallback path is B37-stable, so this is purely a quality upgrade — no credit / correctness impact. Full suite **119 pass** unchanged.

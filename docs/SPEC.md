...
### ~~B26 — Fuzzy `:has-text` selectors on Material Icon tokens silently redirect /edit/ → /project/ between model selection and submit (P0)~~ ✅ FIXED (commit `__B26_COMMIT__`)
...
- **Resolution (commit `__B26_COMMIT__`):** exact-text everywhere — use Playwright's `:text-is(...)` engine (exact, trimmed match) on the Material Icon `<i>` child's textContent. Icon ligatures are designed by Google to be stable across locales, font-loading states, and Flow UI redesigns.

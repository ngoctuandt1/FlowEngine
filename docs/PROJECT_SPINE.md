...
- Recent epics:
  - 2026-05-02 IdeaStudio web clone + perf cluster: 56 PRs (#117-#176). DAG canvas project view, Ý TƯỞNG idea/chat right rail, Setup page (Gemini SDK + Veo accounts + Nano API), multi-track timeline shell, project-first data model + `/api/projects` CRUD, render-compose backend, gold theme. Perf round drops thumbnail bandwidth ~240× (mp4 → poster.jpg) and initial JS payload ~9× (lazy-load page modules). See [session-reports/2026-05-02_ideastudio-clone-perf-cluster.md](session-reports/2026-05-02_ideastudio-clone-perf-cluster.md).
...
- Latest IdeaStudio clone + perf report: [docs/session-reports/2026-05-02_ideastudio-clone-perf-cluster.md](session-reports/2026-05-02_ideastudio-clone-perf-cluster.md).

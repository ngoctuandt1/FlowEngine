...
| Model | Fields / notes |
|---|---|
| `ChainCreate` | Request body: `jobs: list[JobCreate]`, optional `profile`. Current `POST /api/chains` is linear-chain only: each step becomes a child of the previous step, explicit per-step `parent_job_id` values are overwritten, and profile pinning resolves to one effective profile for the whole chain: chain-level `profile` wins when present, otherwise the first non-empty step `JobCreate.profile` is used and all non-empty step profiles must match it. |
...
Wave 5 adds four user-facing surfaces while preserving the server/worker/flow split.

- **Characters entity:** `server/models/character.py` defines reusable `Character` rows with `id`, optional `project_id`, `name`, `ref_image_url`, `voice_id`, `description`, `image_paths`, and timestamps. `server/routes/characters.py` keeps CRUD under `/api/characters`; create/update validates optional project binding and restricts image references to local `/uploads` paths. Prompt-time `@tag` mentions are parsed in `flow/characters.py` and exposed through `flow/operations/character.py`; matching accepts character `tag`, `name`, `id`, first name token, slug, or compact name, while unresolved tags stay visible as validation errors.
- **Trash flow:** Soft delete is represented by nullable `deleted_at` on both `jobs` and `projects`. Normal job/project queries exclude rows with `deleted_at IS NOT NULL`. Explicit trash APIs list deleted rows, `POST /api/trash/restore` restores selected `job_ids`, `project_ids`, or `all=true`, and `DELETE /api/trash/permanent` permanently deletes only trashed rows. Wave 5 smoke coverage calls these mounted HTTP routes directly, not endpoint fallbacks, before store assertions. Flow-side helpers in `flow/trash.py` open `/fx/tools/flow/project/{project_id}/trash` and only click `Restore All` / `Delete All` when caller passes explicit confirmation.
- **Link sharing:** `server/db/share_store.py` persists one share row per job with nullable `share_token`, `share_url`, `shared_at`, and `revoked_at`. `POST /api/jobs/{id}/share` mints or returns the current token, `DELETE /api/jobs/{id}/share` revokes idempotently, and `GET /api/shares/{token}` serves a redacted public job view. `flow/share.py` handles Flow UI share modal extraction and keeps copied links/tokens out of logs except redacted-safe metadata.
- **AI-locator wire-up:** The opt-in fallback is gated by `FLOW_AI_LOCATOR_ENABLED` and only runs after deterministic candidates fail. Current Wave 5 hook points are marketing landing CTA (`flow/landing.py`), model chip/dropdown selection (`flow/model_selector.py`), composer Video tab enforcement (`flow/operations/generate.py`), and edit-view kebab recovery (`flow/edit_menu.py`). Supporting knobs remain `FLOW_AI_LOCATOR_BASE_URL`, `FLOW_AI_LOCATOR_MODEL`, `FLOW_AI_LOCATOR_TIMEOUT_SEC`, and `FLOW_AI_LOCATOR_WIRE`.

...
4. Selector can still tolerate legacy LP labels during rollout, but canonical selection prefers non-LP labels first.
5. `DEFAULT_MODEL` in `flow/model_selector.py` becomes `veo-3.1-lite`; server/frontend defaults wait for Unit E.
6. `generate.py` call sites pass canonical video model keys into selector without changing composer menu structure.
7. Root tests only: update `tests/test_model_selector.py` and `tests/test_model_selector_lite_fallback.py`; do not create `tests/flow/...`.
...
**Runs after Unit A merges.** Unit B receives `flow/operations/generate.py` ownership only after Unit A is merged.
...
8. No L2 count enforcement is added; free-tier L2 composer is paywalled and unobserved for count chips.
9. Tests live at repo-root `tests/test_generate.py`.
...
**READS:**
- `worker/browser_pool.py` to respect existing `reset_for_next_job(target_url=reset_url)` call at `worker/browser_pool.py:133`.
...
3. `flow/client.py:629` `reset_for_next_job(target_url=...)` is the central per-job integration point. If `target_url` contains a Flow project id, navigate, handle landing/login recovery as needed, then disable Agent before handing client back.
4. If `target_url` does not contain a project id, reset remains buffer cleanup only; do not invent project state.
5. Every Agent mutation logs profile, project id, previous detection state, method (`reverseAPI` or `DOM`), and whether restoration token is available.
6. Expose `restore_agent_state` helper for opt-in restoration, but do not auto-restore by default unless user config enables it.
7. 403/404/reverseAPI failures are nonfatal only if DOM fallback confirms normal composer controls are visible; otherwise fail with clear diagnostic.
8. Unit F integration smoke covers Agent already-off, DOM fallback, and mutation logging because Unit C is forbidden from editing tests.
...
**OWNS in Wave 1:**
- `flow/operations/_base.py`
- `flow/operations/_multitab.py`
- `worker/dispatcher.py` — only L2 batch result inspection path around `worker/dispatcher.py:1288` through `worker/dispatcher.py:1315`
...
- `worker/dispatcher.py` LP default lines later owned by Unit E

**Acceptance criteria:**
1. Define `L2PaywallError` in `flow/operations/_base.py` alongside `LeafLockoutError`; do not create `flow/exceptions.py`.
...
6. Dispatcher uses canonical shape: `error_kind="paid_tier_required"`; operation is carried separately via existing `job.type` or explicit result field, not concatenated as `paid_tier_required_extend`.
7. Profile is not burned, job is not retried, and recaptcha/profile-swap paths are not invoked for paywall.
8. `_multitab.py` re-raises `L2PaywallError` before the generic `Exception` catch currently returning `{status: "failed", error: str(exc)}` at `flow/operations/_multitab.py:366` through `flow/operations/_multitab.py:374`.
9. L2 batch dispatcher path at `worker/dispatcher.py:1309` explicitly inspects tab results/exceptions and preserves `paid_tier_required` for affected jobs.
10. If paid-tier L2 composer later exposes count chips, do not implement here; file a follow-up after live paid capture.
11. Tests cover DOM banner detection, non-paywall missing-button diagnostic, single-job dispatcher handling, and multitab/batch propagation.
...
**Runs after Unit D merges** because both units touch `worker/dispatcher.py` in different ranges. Unit E owns LP-default dispatcher lines only after Unit D is merged.
...
- `worker/dispatcher.py` — LP default refs near `worker/dispatcher.py:257`, `worker/dispatcher.py:290`, `worker/dispatcher.py:361`, `worker/dispatcher.py:393`; do not edit Unit D L2 batch logic except conflict resolution after merge
...
- Unit B blocks submit if count is not one or if preview exceeds configured budget.
...
- `D -> E` because both edit `worker/dispatcher.py`; Unit D owns L2 batch path first, Unit E owns LP default refs after D merges.
- `C` stands alone.
...
|---|---|---:|---|
| 1 | A, C, D | 3 | No simultaneous file overlap |
| 2 | B | 1 | Starts after A merge |
